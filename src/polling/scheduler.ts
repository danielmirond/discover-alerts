...
import { runBoePoll } from './boe-poll.js';
...
export function startPolling(): { stop: () => void } {
  console.log(
    ...
    `BOE every ${config.polling.boeIntervalMs / 1000}s`,
  );
  ...
  const initialBoe = setTimeout(safeRun('boe', runBoePoll), 15000);
  ...
  const boeInterval = setInterval(
    safeRun('boe', runBoePoll),
    config.polling.boeIntervalMs,
  );
